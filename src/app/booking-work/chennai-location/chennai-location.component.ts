...
export class ChennaiLocationComponent  implements OnInit{
  ...
  constructor(private router:Router,private booking:BookingService,private cartService:CartService,private UserBooked:UserBookedHistoryService){}
    ...
    addtocartdis(item:any){
      ...
      this.UserBooked.cart(this.cartdis).subscribe(res=>{
        ...
        if(!validate){
          ...
          this.cartdis.addtocartdis(item.subscribe());
        }
      })
    }
}
